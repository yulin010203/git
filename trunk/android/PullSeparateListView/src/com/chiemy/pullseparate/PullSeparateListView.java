...
public class PullSeparateListView extends ListView {
	 ...
	 * 
	 * @param separateAll
	 *            如果为true,那么全部都会分离。否则的话，如果是顶部下拉，只有点击位置之前的Item会分离</br> 如果是底部上拉，则只有点击位置之后的item会分离。默认为false
	 */
	...
	private void recoverDownView() {
		if (showDownAnim && downView != null) {
			ViewPropertyAnimator.animate(downView).scaleX(1f).scaleY(1f).setDuration(separate ? SEPARATE_RECOVER_DURATION : 100).setInterpolator(new AccelerateInterpolator());
		}
	}
