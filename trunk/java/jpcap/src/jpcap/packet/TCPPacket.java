...
public class TCPPacket extends IPPacket {
	...
	void setValue(int src, int dst, long seq, long ack_num, boolean urg,
			...
			boolean rsv1, boolean rsv2, int win, short urp) {
		src_port = src;
		dst_port = dst;
		sequence = seq;
		this.ack_num = ack_num;
	 ...
	 * 
	 * <BR>
	 * Format: src_port > dst_port seq(sequence) win(window) [ack ack_num]
	 * [S][F][P]
	 * 
	 * @return a string representation of this packet
	 */
	public String toString() {
		return super.toString() + " TCP " + src_port + " > " + dst_port
				+ " seq(" + sequence + ") win(" + window + ")"
				+ (ack ? " ack " + ack_num : "") + " " + (syn ? " S" : "")
				+ (fin ? " F" : "") + (psh ? " P" : "") + (rst ? " R" : "")
				+ (urg ? " U" : "");
	}
}
