 ...
 * 
 * @param <E>
 *            implementation storing the data for sharing during exchange or
...
public final class RingBuffer<E> implements Cursored, DataProvider<E> {
	 ...
	 * 
	 * @param eventFactory
	 *            to newInstance entries for filling the RingBuffer
	 * @param sequencer
	 *            sequencer to handle the ordering of events moving through the
	 *            RingBuffer.
	 * @throws IllegalArgumentException
	 *             if bufferSize is less than 1 or not a power of 2
	 */
	 ...
	 * @param factory
	 *            used to create the events within the ring buffer.
	 * @param bufferSize
	 *            number of elements to create within the ring buffer.
	 * @param waitStrategy
	 *            used to determine how to wait for new elements to become
	 *            available.
	 * @throws IllegalArgumentException
	 *             if bufferSize is less than 1 or not a power of 2
	 */
	 ...
	 *            number of elements to create within the ring buffer.
	 * @throws IllegalArgumentException
	 *             if <tt>bufferSize</tt> is less than 1 or not a power of 2
	 */
	 ...
	 * 
	 * @param producerType
	 *            producer type to use {@link ProducerType}.
	 * @param factory
	 *            used to create events within the ring buffer.
	 * @param bufferSize
	 *            number of elements to create within the ring buffer.
	 * @param waitStrategy
	 *            used to determine how to wait for new elements to become
	 ...
	 * {@link SequenceBarrier#waitFor(long)} method.
	 * </p>
	 * 
	 * @param sequence
	 *            for the event
	 ...
	 * but it is worth not that it is a racy thing to do and should only be used
	 * in controlled circumstances. E.g. during initialisation.
	 * 
	 * @param sequence
	 *            The sequence to reset too.
	 * @throws IllegalStateException
	 *             If any gating sequences have already been specified.
	 */
	 ...
	 * that is stored there. This is another deliberately racy call, that should
	 * only be done in controlled circumstances, e.g. initialisation.
	 * 
	 * @param sequence
	 *            The sequence to claim.
	 ...
	 * 
	 * @param sequence
	 *            The sequence to identify the entry.
	 ...
	 *            the additional sequences to track
	...
	/**
	 * Get the current cursor value for the ring buffer. The cursor value is the
	 * last value that was published, or the highest available sequence that can
	 * be consumed.
	 */
	 ...
	 * Especially true if this ring buffer is set up to handle multiple
	 * producers.
	 * 
	 * @param requiredCapacity
	 *            The capacity to check for.
	 * @return <tt>true</tt> If the specified <tt>requiredCapacity</tt> is
	 *         available <tt>false</tt> if now.
	 */
	 ...
	 * next sequence, getting the current (uninitialised) event from the ring
	 * buffer and publishing the claimed sequence after translation.
	 * 
	 * @param translators
	 ...
	 *            The user specified translation for each event
	 * @param batchStartsAt
	 *            The first element of the array which is within the batch.
	 * @param batchSize
	 ...
	 * @see Sequencer#next(int)
	 * @param lo
	 *            the lowest sequence number to be published
	 * @param hi
	 *            the highest sequence number to be published
	 */
	...
	private <A> void batchOverRuns(final A[] arg0, final int batchStartsAt, final int batchSize) {
		if (batchStartsAt + batchSize > arg0.length) {
			throw new IllegalArgumentException("A batchSize of: " + batchSize + " with batchStatsAt of: " + batchStartsAt + " will overrun the available number of arguments: "
	...
	private <A, B, C> void translateAndPublishBatch(final EventTranslatorThreeArg<E, A, B, C> translator, final A[] arg0, final B[] arg1, final C[] arg2, int batchStartsAt, final int batchSize,
