...
/**
 * <p>
 * Histogram for tracking the frequency of observations of values below interval
 * upper bounds.
 * </p>
 * 
 * <p>
 * This class is useful for recording timings across a large number of
 * observations when high performance is required.
 * <p>
 * 
 * <p>
 * The interval bounds are used to define the ranges of the histogram buckets.
 * If provided bounds are [10,20,30,40,50] then there will be five buckets,
 * accessible by index 0-4. Any value 0-10 will fall into the first interval
 * bar, values 11-20 will fall into the second bar, and so on.
 * </p>
 */
public final class Histogram {
	 ...
	 * @throws IllegalArgumentException
	 *             if interval count or values do not match exactly
	 */
	 ...
	 * 
	 * <p>
	 * The mean is calculated by summing the mid points of each interval
	 * multiplied by the count for that interval, then dividing by the total
	 * count of observations. The max and min are considered for adjusting the
	 * top and bottom bin when calculating the mid point, this minimises skew if
	 * the observed values are very far away from the possible histogram values.
	 * </p>
	 * 
	 ...
	 * 
	 * <p>
	 * Note this does not get the actual percentile measurement, it only gets
	 * the bucket
	 * </p>
	 * 
	 * @param factor
